...
public class BoardController {
	 ...
	 * 
	 * @RequestParam을 이용하면 특정값을 가져올수 있다 그 특정값을 찾아 int bno에 넣어주고 Model을 이용하여 뷰(view)에
	 * 데이터를 넘겨준다.
	 */
	...
	public void getListPage(Model model, @RequestParam(value = "num", defaultValue = "1") int num) throws Exception {
		...
		/*
		 * Page 클래스가 있으므로 하나로 모델처리함. model.addAttribute("pageNum", page.getPageNum());
		 * 
		...
		/*
		 * Page 클래스를 만들지 않고 페이징 처리 하는 방법. // 게시물 총 갯수 int count = service.count();
		 * 
		 * // 한 페이지에 출력할 게시물 갯수 int postNum = 10;
		 * 
		 * // 화면 화단에 표시할 페이징 번호 구하기 . ([ 게시물 총 갯수 ÷ 한 페이지에 출력할 갯수 ]의 올림) int pageNum =
		 * (int)Math.ceil((double)count/postNum);
		 * 
		 * //현재 페이지 기준으로 10개의 게시글 구하기. // ((현재페이지번호 - 1) * 출력할 게시글 갯수 ) + 1 int
		 * displayPost = ((num -1) * postNum) + 1;
		 * 
		 * // 화면 이동마다 10개씩 데이터 출력을 위해 다시 postNum을 초기화 후 페이징번호 * 10을 구함. postNum = 0;
		 * postNum = 10*(num);
		 * 
		 ...
		 * 
		 * // 10개씩 하단에 표시되는 페이지 번호 중 마지막 번호 (*** 1차 계산 ***) // 마지막 페이징 번호 = ((올림)(현재 페이지
		 * 번호 / 한번에 표시할 페이지 번호의 갯수)) * 한번에 표시할 페이지 번호의 갯수 int endPageNum =
		 * (int)(Math.ceil((double)num / (double)pageNum_cnt) * pageNum_cnt);
		 * 
		 * // 표시되는 페이징 번호중 첫번째 번호 int startPageNum = endPageNum - (pageNum_cnt - 1);
		 * 
		 * //하단 페이징 마지막 번호 재계산 (*** 2차 계산 ***) int endPageNum_tmp =
		 * (int)(Math.ceil((double)count / (double)pageNum_cnt)); // 현재 페이징 번호(1차계산)가
		 * 하단에 표시될 마지막번호(2차계산)보다 크면 2차로 계산한 값을 넣어준다. if(endPageNum > endPageNum_tmp) {
		 * endPageNum = endPageNum_tmp; }
		 * 
		 * //이전버튼을 만들기 위한.. boolean prev = startPageNum == 1 ? false : true; //다음버튼 만들기
		 * 위한.. boolean next = endPageNum * pageNum_cnt >= count ? false : true;
		 * 
		 ...
		 * next);
		 * 
		 * // 현재페이지 model.addAttribute("select", num);
		 */
	}
